...
public class SelfPubActivity extends BaseActivity implements View.OnClickListener,LoadMoreListView.OnLoadMore {
    ...
    //提供外部进入该activity的方法
    public static void enterSelfPubAty(Context context){
        Intent i=new Intent(context,SelfPubActivity.class);
    ...
    private void initView() {
        ...
        tv_title.setText(R.string.personcenter_down_article);
...
//                    ShowToast.show(SelfPubActivity.this, "加载数据成功！");
                    break;
                case GET_MORE_DATA_FAIL:
                    mListView.onLoadComplete();
                    ShowToast.show(SelfPubActivity.this, "暂无更多数据！");
                    break;
            }
        }
    };
    ...
    private void getMoreData(String joke_id,String token,int count){
        new GetUserAboutJokeNet(joke_id, token, count + "",MyConfig.USER_ABOUT_JOKE_TYPE_SELFPUB ,new GetUserAboutJokeNet.SuccessGetUserJokeCallback() {
            @Override
