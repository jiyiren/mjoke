...
public class Tab01Fragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener,LoadMoreListView.OnLoadMore{
...
//    private ProgressDialog dialog;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        mview=LayoutInflater.from(getActivity()).inflate(R.layout.fragment_tab01,null);
...
//        dialog.setCancelable(false);
        swipeRefreshLayout= (SwipeRefreshLayout) mview.findViewById(R.id.tab01_swipe_ly);
        swipeRefreshLayout.setColorSchemeResources(android.R.color.holo_blue_light,
                android.R.color.holo_red_light, android.R.color.holo_orange_light,
                android.R.color.holo_green_light);
        swipeRefreshLayout.setOnRefreshListener(this);
        mListView= (LoadMoreListView) mview.findViewById(R.id.tab01_lv);
...
//                    ShowToast.show(getActivity(), "更新数据成功！");
                ...
                case GETJOKE_FAIL:
                    ...
                    ShowToast.show(getActivity(), "还没有最新数据哟~~");
                    break;
...
//                    mListDatas=g.fromJson(data, new TypeToken<List<SingleJoke>>(){}.getType());
    ...
    private void getMoreData(String joke_id,int count){
        new GetJokesNet(joke_id, MyConfig.SCROLL_LOADINGMORE, count,MyConfig.FRAGMENT_TYPE_NEW,new GetJokesNet.SuccessGetJokeCallback() {
            @Override
            public void onSuccess(String result) {
                String res= MyUtils.getGson(result,MyConfig.KEY_RESULT);
                if(res.equals(MyConfig.RESULT_SUCCESS)){
